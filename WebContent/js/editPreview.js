								...
								$(".tagPanel")
										.append(
												"<label class='tagStyle'>"
														+ tag
														+ "<label class='delTag'><a href='javascript:void(0)' onclick='delTag(this)'>X</a></label>"
				...
				success : function(data) {
					if (data != "") {
						...
						if (data != "{}") {
							...
							$(".filePanel")
									.html(
											"<img src='" + jsonObj.result.head
													+ "' />");
							$("input[name='head']").val(jsonObj.result.head);
							$("input[name='nickName']").val(
									jsonObj.result.nickName);
							$("input[name='mobile']")
									.val(jsonObj.result.mobile);
							$(
									"input[type=radio][value="
											+ jsonObj.result.gender + "]")
									.attr("checked", 'checked');
							$("input[name='birthday']").val(
									jsonObj.result.birthday);
							if (jsonObj.result.birthday != null) {
								var birth = jsonObj.result.birthday.split('-',
										-1);
								$("#js-year").val(birth[0]);
							...
							// 标签
							for (var i = 0, tagLen = jsonObj.result.tags
									.split(',').length; i < tagLen; i++) {
							...
							.uploader({
								...
								init : {
									...
									'FileUploaded' : function(up, file, info) {
										...
										var sourceLink = "http://" + domain
												+ "/" + res.key; // 获取上传成功后的文件的Url
										$(".filePanel")
												.html(
														"<img src='"
																+ sourceLink
																+ "' onload='AutoResizeImage(150,150,this)' />");
